  ...
  document.querySelectorAll('.sb-link[data-page="dept"]').forEach(link => {
    ...
    if (deptId === 'laboratorium') {
      ...
      ul.innerHTML += `<li onclick="syncAkademikFromSumber()" style="color:#22c55e; font-weight:600;">
        <i class="fas fa-rotate" id="sbAkademikSyncIcon"></i> <span style="flex:1;">Sinkron Data Live Akademik</span>
      </li>`;
      ul.innerHTML += `<li onclick="window.open('https://bidang-administrasi-akademik.web.app', '_blank')" style="color:#3b82f6; font-weight:600;">
  ...
  Object.keys(DEPT).forEach(k => {
    ...
    if (k === 'akademik') {
      ...
      html += `<li onclick="syncAkademikFromSumber()" style="color:#22c55e; font-weight:600;">
        <i class="fas fa-rotate" id="sbAkademikSyncIcon"></i> <span style="flex:1; min-width:0; line-height:1.4;">Sinkron Data Live Akademik</span>
      </li>`;
      ul.innerHTML = html;
      navLink.after(ul);
      return;
...
// Dataset Standar Bersih SIM-PKM AAS Prodi D-III Akupunktur (Default Kosong / Real Data Only)
    ...
    if (saved) {
      ...
      if (parsed && typeof parsed === 'object') {
        return {
          ...
          dokumen: Array.isArray(parsed.dokumen) ? parsed.dokumen : []
        };
      }
    }
...
function initPengabdianCharts() {
  ...
  const skemaCounts = {};
  proposals.forEach(p => {
    const s = p.scheme || 'PkM Reguler Internal AAS';
    skemaCounts[s] = (skemaCounts[s] || 0) + 1;
  });
  const hasSkemaData = Object.keys(skemaCounts).length > 0;
  const skemaLabels = hasSkemaData ? Object.keys(skemaCounts) : ['Belum Ada Usulan (0 Data)'];
  const skemaValues = hasSkemaData ? Object.values(skemaCounts) : [1];
  const skemaColors = hasSkemaData ? ['#eab308', '#3b82f6', '#10b981', '#ec4899', '#8b5cf6'] : ['#cbd5e1'];
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: function(ctx) {
              if (!hasSkemaData) return ' 0 Usulan PkM';
              return ' ' + ctx.label + ': ' + ctx.raw + ' Usulan';
            }
          }
        }
      }
  ...
  chartPkmTCMInstance = new Chart(ctxTCM.getContext('2d'), {
    ...
    options: {
      ...
      scales: {
        y: { beginAtZero: true, suggestedMax: 5, ticks: { stepSize: 1, precision: 0 } }
      }
    }
  });
  ...
  if (ctxVAS) {
    ...
    if (patients.length === 0) {
      kasusLabels = ['Belum Ada Data Pasien (0 Data)'];
      vasPreValues = [0];
      vasPostValues = [0];
    } else {
      const sample = patients.slice(0, 6);
      sample.forEach(p => {
        const shortName = p.name ? p.name : 'Pasien';
        const diag = p.complaint ? p.complaint.slice(0, 16) : 'Baksos';
        kasusLabels.push(shortName + ' (' + diag + '...)');
        vasPreValues.push(Number(p.vas_pre) || 0);
        vasPostValues.push(Number(p.vas_post) || 0);
      });
    }
    ...
    chartPkmVASInstance = new Chart(ctxVAS.getContext('2d'), {
      ...
      options: {
          ...
          tooltip: {
            callbacks: {
              afterLabel: function(ctx) {
                if (ctx.datasetIndex === 1 && vasPreValues[ctx.dataIndex] > 0) {
        ...
        scales: {
          y: { beginAtZero: true, max: 10, ticks: { stepSize: 2 }, title: { display: true, text: 'Skala VAS (0-10)' } }
        }
      }
    });
  }
...
function renderPengabdianReviewerTable() {
  ...
  if (list.length === 0) {
    tbody.innerHTML = '<tr><td colspan="8" style="text-align:center; padding:30px; color:var(--t3);"><i class="fas fa-clipboard-check" style="font-size:2rem; margin-bottom:8px; display:block;"></i>Belum ada data review proposal PkM (0 Data)</td></tr>';
    return;
  }

  if (list.length === 0) {
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding:30px; color:var(--t3);"><i class="fas fa-award" style="font-size:2rem; margin-bottom:8px; display:block;"></i>Belum ada luaran publikasi atau HKI (0 Data)</td></tr>';
    return;
  }

  if (list.length === 0) {
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding:30px; color:var(--t3);"><i class="fas fa-folder-open" style="font-size:2rem; margin-bottom:8px; display:block;"></i>Tidak ada berkas / dokumen SK PkM (0 Data)</td></tr>';
    return;
  }
...
function renderPengabdianBorangTable() {
  ...
  const proposals = pengabdianData.proposals || [];

  if (currentBorangSubTab === '71') {
    let rowsHtml = '';
    if (proposals.length === 0) {
      rowsHtml = '<tr><td colspan="7" style="text-align:center; padding:30px; color:var(--t3);"><i class="fas fa-inbox" style="font-size:2rem; margin-bottom:8px; display:block;"></i>Belum ada data kegiatan Pengabdian kepada Masyarakat (PkM) DTPS (0 Data)</td></tr>';
    } else {
      rowsHtml = proposals.map((p, idx) => {
        const mhs = Array.isArray(p.members_student)
          ? p.members_student.map(m => typeof m === 'object' ? m.name : m).join(', ')
          : '-';
        return `
          <tr>
            <td style="text-align:center;">${idx + 1}</td>
            <td><strong>${p.leader || '-'}</strong></td>
            <td>${p.scheme || 'Akupunktur Terapan'}</td>
            <td>${mhs}</td>
            <td>${p.title}</td>
            <td style="text-align:center;">${(p.date || '').slice(0, 4) || '2026'}</td>
            <td>${p.scheme} (Rp ${(Number(p.budget) || 0).toLocaleString('id-ID')})</td>
          </tr>
        `;
      }).join('');
    }
    ...
    const proposalsWithPartner = proposals.filter(p => p.partner);
    let rowsHtml = '';
    if (proposalsWithPartner.length === 0) {
      rowsHtml = '<tr><td colspan="6" style="text-align:center; padding:30px; color:var(--t3);"><i class="fas fa-handshake-slash" style="font-size:2rem; margin-bottom:8px; display:block;"></i>Belum ada data kerjasama mitra PkM (0 Data)</td></tr>';
    } else {
      rowsHtml = proposalsWithPartner.map((p, idx) => `
        <tr>
          <td style="text-align:center;">${idx + 1}</td>
          <td><strong>${p.partner}</strong></td>
          <td>Lokal (Kota Surabaya)</td>
          <td>${p.title}</td>
          <td>MoA / Kontrak PkM No. ${p.id}</td>
          <td>Wahana Praktik Klinis & Pembelajaran Lapangan D-III Akupunktur</td>
        </tr>
      `).join('');
    }
    ...
    let studentRows = [];
    proposals.forEach(p => {
      if (Array.isArray(p.members_student)) {
        p.members_student.forEach(m => {
          studentRows.push({
            name: typeof m === 'object' ? m.name : m,
            nim: typeof m === 'object' ? (m.nim || '-') : '-',
            leader: p.leader,
            title: p.title
          });
        });
    ...
    if (studentRows.length === 0) {
      rowsHtml = '<tr><td colspan="6" style="text-align:center; padding:30px; color:var(--t3);"><i class="fas fa-user-graduate" style="font-size:2rem; margin-bottom:8px; display:block;"></i>Belum ada data pelibatan mahasiswa dalam PkM (0 Data)</td></tr>';
    } else {
      rowsHtml = studentRows.map((s, idx) => `
        <tr>
          <td style="text-align:center;">${idx + 1}</td>
          <td><strong>${s.name}</strong></td>
          <td>${s.nim}</td>
          <td>${s.leader}</td>
          <td>Pelaksana Skrining &amp; Terapi Baksos: ${s.title}</td>
    ...
    const outputs = pengabdianData.outputs || [];
    let rowsHtml = '';
    if (outputs.length === 0) {
      rowsHtml = '<tr><td colspan="5" style="text-align:center; padding:30px; color:var(--t3);"><i class="fas fa-book-open" style="font-size:2rem; margin-bottom:8px; display:block;"></i>Belum ada data integrasi hasil PkM ke pembelajaran (0 Data)</td></tr>';
    } else {
      rowsHtml = outputs.map((o, idx) => `
        <tr>
          <td style="text-align:center;">${idx + 1}</td>
          <td>${o.judul || '-'}</td>
          <td>Akupunktur Terapan &amp; Klinis</td>
          <td>Modul Praktikum &amp; Studi Kasus Pembelajaran</td>
          <td>${o.tahun || '2026'}</td>
        </tr>
      `).join('');
    }
...
async function clearAllPengabdianData() {
  if (!confirm("Hapus seluruh data dummy / reset data Bidang Pengabdian Masyarakat ke 0?\n\nTindakan ini akan mengosongkan usulan proposal, rekam pasien baksos, luaran, dan menyinkronkan seluruh grafik analitik ke angka 0.")) {
    return;
  }

  // 1. Reset memori lokal pengabdianData
  pengabdianData = {
    proposals: [],
    patients: [],
    outputs: [],
  ...
  // 3. Bersihkan record dummy SIMPKM dari arsip SIMARSIP
  if (typeof arsip !== 'undefined' && Array.isArray(arsip)) {
    arsip = arsip.filter(a => !a.id || (!a.id.startsWith('SIMPKM-') && !a.id.startsWith('PKM-')));
    if (typeof save === 'function') save();
  }

  // 4. Reset remote Firestore jika terhubung
  try {
    ...
    if (dbPkmSumber) {
      await dbPkmSumber.collection('sim_pkm_data').doc('main_store').set({
        proposals: [],
        patients: [],
        outputs: [],
        reviews: [],
    ...
    console.warn("Reset remote Firestore store info:", e);
  }

  // 5. Update tampilan indikator & grafik ke 0
  if (typeof updateBadges === 'function') updateBadges();
  renderPengabdianContent();

  if (typeof toast === 'function') {
    toast("Semua data dummy berhasil dihapus! Seluruh grafik & indikator disinkronkan ke angka 0.", "success");
  } else {
    alert("Semua data dummy berhasil dihapus! Seluruh grafik & indikator disinkronkan ke angka 0.");
  }
}
...
async function syncPengabdianFromSumber(silent = false) {
  ...
  try {
    ...
    if (syncedData) {
      pengabdianData.proposals = Array.isArray(syncedData.proposals) ? syncedData.proposals : [];
      pengabdianData.patients = Array.isArray(syncedData.patients) ? syncedData.patients : [];
      pengabdianData.outputs = Array.isArray(syncedData.outputs) ? syncedData.outputs : [];
      pengabdianData.reviews = Array.isArray(syncedData.reviews) ? syncedData.reviews : [];
      pengabdianData.dokumen = Array.isArray(syncedData.dokumen) ? syncedData.dokumen : [];
    }

    // Bersihkan record proposal yang dihapus dari arsip SIMARSIP
    const validProposalIds = new Set((pengabdianData.proposals || []).map(p => 'SIMPKM-' + p.id));
    if (typeof arsip !== 'undefined' && Array.isArray(arsip)) {
      arsip = arsip.filter(a => !a.id || !a.id.startsWith('SIMPKM-') || validProposalIds.has(a.id));
    }

    // Upsert usulan proposal aktif ke SIMARSIP arsip collection
    let countUpsert = 0;
...
// ══════════════════════════════════════════════════════════════
// ADMINISTRASI AKADEMIK (SIAKAD AAS) INTEGRATION LOGIC
// ══════════════════════════════════════════════════════════════
let currentAkademikTab = 'dashboard';
let akChartSemesterInst = null;
let akChartKelasInst = null;
let akChartGenderInst = null;
let akChartStatusInst = null;
let dbAkademikSumber = null;
let akademikListenerAttached = false;
...
// Data cadangan jika database cloud baru/kosong
const FALLBACK_AKADEMIK_STUDENTS = [
  { id: '1', nim: '2301001', name: 'Ahmad Fauzi', gender: 'L', prodi: 'D-III Akupunktur (Reguler)', semester: 3, status: 'Aktif' },
  { id: '2', nim: '2301002', name: 'Siti Nurhaliza', gender: 'P', prodi: 'D-III Akupunktur (Reguler)', semester: 3, status: 'Aktif' },
  { id: '3', nim: '2201015', name: 'Bambang Pratama', gender: 'L', prodi: 'D-III Akupunktur (Karyawan/Alih Jenjang)', semester: 5, status: 'Aktif' },
  { id: '4', nim: '2401008', name: 'Dewi Lestari', gender: 'P', prodi: 'D-III Akupunktur (Reguler)', semester: 1, status: 'Aktif' },
  { id: '5', nim: '2101030', name: 'Eko Prasetyo', gender: 'L', prodi: 'D-III Akupunktur (Reguler)', semester: 6, status: 'Lulus' }
];

function switchAkademikTab(tabKey) {
  currentAkademikTab = tabKey || 'dashboard';

  const akContainer = document.getElementById('akademikContainer');
  if (akContainer) akContainer.style.display = 'block';

  // Toggle tab views
  const views = [
    { key: 'dashboard', id: 'akademikView-dashboard' },
    { key: 'mahasiswa', id: 'akademikView-mahasiswa' },
    { key: 'arsip', id: 'akademikView-arsip' },
    { key: 'portal', id: 'akademikView-portal' }
  ];

  views.forEach(v => {
    const el = document.getElementById(v.id);
    if (el) el.style.display = (v.key === currentAkademikTab) ? 'block' : 'none';

    const btn = document.getElementById(`btnAkademikTab-${v.key}`);
    if (btn) {
      if (v.key === currentAkademikTab) {
        btn.classList.add('active');
        btn.style.background = 'var(--primary)';
        btn.style.color = '#fff';
      } else {
        btn.classList.remove('active');
        btn.style.background = 'transparent';
        btn.style.color = (v.key === 'portal') ? '#3b82f6' : 'var(--t1)';
      }
    }
  });
  ...
  if (sbMenu) {
    ...
    lis.forEach(li => {
      const onclickAttr = li.getAttribute('onclick') || '';
      if (onclickAttr.includes(`'${currentAkademikTab}'`)) {
        li.classList.add('active');
      } else if (!onclickAttr.includes('window.open')) {
        li.classList.remove('active');
      }
  ...
  } else if (currentAkademikTab === 'portal') {
    const iframe = document.getElementById('akademikIframe');
    if (iframe && (!iframe.src || (!iframe.src.includes('bidang-administrasi-akademik.web.app') && !iframe.src.includes('workers.dev')))) {
      iframe.src = 'https://bidang-administrasi-akademik.web.app';
    }
  }
}

function switchAkademikTabFromSidebar(tabKey, el) {
...
function reloadAkademikFrame() {
  const iframe = document.getElementById('akademikIframe');
  if (iframe) {
    iframe.src = iframe.src || 'https://bidang-administrasi-akademik.web.app';
    try { iframe.contentWindow?.location.reload(); } catch(e) { iframe.src = iframe.src; }
  }
}
...
function updateAkademikStats() {
  ...
  const aktif = list.filter(s => (s.status || '').toLowerCase() === 'aktif').length;
  const reguler = list.filter(s => (s.prodi || '').includes('Reguler')).length;
  ...
  const elAktif = document.getElementById('ak-stat-aktif');
  if (elAktif) elAktif.textContent = aktif;

  const elKelas = document.getElementById('ak-stat-kelas');
  if (elKelas) elKelas.textContent = `${reguler} : ${alihJenjang}`;

  const elGender = document.getElementById('ak-stat-gender');
  if (elGender) elGender.textContent = `${laki} : ${perempuan}`;

  const elArsip = document.getElementById('ak-stat-arsip');
...
function renderAkademikMhsTable() {
  const tbody = document.getElementById('akMhsTableBody');
  if (!tbody) return;

  const filterSem = (document.getElementById('akFilterSemester')?.value || '').trim();
  const filterStat = (document.getElementById('akFilterStatus')?.value || '').trim();
  const search = (document.getElementById('akMhsSearch')?.value || '').toLowerCase().trim();
  ...
  const filtered = list.filter(item => {
    if (filterSem && String(item.semester) !== filterSem) return false;
    if (filterStat && (item.status || '').toLowerCase() !== filterStat.toLowerCase()) return false;
    if (search) {
      const matchNim = (item.nim || '').toLowerCase().includes(search);
      const matchName = (item.name || '').toLowerCase().includes(search);
      const matchProdi = (item.prodi || '').toLowerCase().includes(search);
      return matchNim || matchName || matchProdi;
    }
    return true;
  });

  if (filtered.length === 0) {
    tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; padding:24px; color:var(--t3);"><i class="fas fa-inbox"></i> Tidak ada data mahasiswa yang sesuai filter.</td></tr>`;
    return;
  }

  tbody.innerHTML = filtered.map((s, idx) => {
    const isAktif = (s.status || '').toLowerCase() === 'aktif';
    const statusBg = isAktif ? '#dcfce7' : '#fef3c7';
    const statusColor = isAktif ? '#15803d' : '#b45309';

    return `<tr>
      <td style="text-align:center; font-weight:600; color:var(--t3);">${idx + 1}</td>
      <td><span style="font-family:monospace; font-weight:700; color:var(--primary);">${s.nim || '-'}</span></td>
      <td><strong>${s.name || '-'}</strong></td>
      <td style="text-align:center;">${s.gender === 'L' ? '<span class="badge" style="background:#e0f2fe; color:#0369a1;">L</span>' : '<span class="badge" style="background:#fce7f3; color:#be185d;">P</span>'}</td>
      <td><span style="font-size:0.85rem; color:var(--t2);">${s.prodi || 'D-III Akupunktur'}</span></td>
      <td style="text-align:center;"><span class="badge" style="background:var(--bg3); color:var(--t1);">Semester ${s.semester || '-'}</span></td>
      <td><span class="badge" style="background:${statusBg}; color:${statusColor};"><i class="fas ${isAktif ? 'fa-check-circle' : 'fa-clock'}"></i> ${s.status || 'Aktif'}</span></td>
    </tr>`;
  }).join('');

  const foot = document.getElementById('akMhsTableFoot');
  if (foot) {
    foot.textContent = `Menampilkan ${filtered.length} dari total ${list.length} mahasiswa`;
  }
}

function renderAkademikArsipTable() {
  const tbody = document.getElementById('akArsipTableBody');
  if (!tbody) return;

  const filterKat = (document.getElementById('akFilterArsipKategori')?.value || '').trim();
  const search = (document.getElementById('akArsipSearch')?.value || '').toLowerCase().trim();
  ...
  if (filtered.length === 0) {
    tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; padding:24px; color:var(--t3);"><i class="fas fa-folder-open"></i> Belum ada dokumen arsip akademik yang diunggah di database.</td></tr>`;
    return;
  }
  ...
  const foot = document.getElementById('akArsipTableFoot');
  if (foot) {
    foot.textContent = `Menampilkan ${filtered.length} dari total ${list.length} berkas arsip`;
  }
}

async function syncAkademikFromSumber(silent = false) {
  const btn = document.getElementById('btnSyncAkademik');
  const icon = document.getElementById('akademikSyncBtnIcon');
  const sbIcon = document.getElementById('sbAkademikSyncIcon');
  ...
  if (!silent && typeof toast === 'function') {
    toast('Menyambungkan real-time ke Database SIAKAD AAS...', 'info');
  }
  ...
  try {
    ...
    if (!silent && typeof toast === 'function') {
      toast('Sinkronisasi Realtime SIAKAD AAS Aktif!', 'success');
    }
  } catch (err) {
    console.error("Sinkronisasi Akademik error:", err);
    if (!silent && typeof toast === 'function') {
      toast('Gagal menyinkronkan data: ' + err.message, 'error');
    }
  } finally {
    if (btn && !silent) {
      btn.disabled = false;
...
function getSimRTData() {
  try {
    ...
    if (raw) {
      ...
      if (parsed) {
        // Bersihkan sisa data dummy lama jika terdeteksi di browser
        if (parsed.pettyCashTransactions && parsed.pettyCashTransactions.some(t => t.id === 'PC-20260901-001' || t.voucherNo === 'BPKK/AAS/2026/09/001' || t.amount === 5000000)) {
          localStorage.removeItem('AAS_SIM_RUMAH_TANGGA_V1');
          return { pettyCashTransactions: [], budgets: [], bills: [], inventoryATK: [], cashOpnameReports: [] };
        }
        if (parsed.budgets && parsed.budgets.some(b => b.allocated === 4500000 || b.allocated === 18000000)) {
          localStorage.removeItem('AAS_SIM_RUMAH_TANGGA_V1');
          return { pettyCashTransactions: [], budgets: [], bills: [], inventoryATK: [], cashOpnameReports: [] };
        }
        return parsed;
...
function initRumahTanggaCharts() {
  const data = getSimRTData();
  const gridEl = document.getElementById('rtChartsGrid');
  const emptyEl = document.getElementById('rtChartsEmptyState');
  ...
  (data.bills || []).forEach(b => {
    ...
    if (amt > 0) {
      billLabels.push(b.utilityType || 'Utilitas');
      billValues.push(amt);
    }
  });

  const hasCashData = (debit > 0 || kredit > 0);
  const hasBillData = billValues.length > 0;
  const hasAnyData = hasCashData || hasBillData;

  // Jika data transaksi masih 0 / kosong, HANCURKAN grafik dan sembunyikan container
  if (!hasAnyData) {
    if (rtCashFlowChartInstance) {
      rtCashFlowChartInstance.destroy();
      rtCashFlowChartInstance = null;
    }
    if (rtExpensePieChartInstance) {
      rtExpensePieChartInstance.destroy();
      rtExpensePieChartInstance = null;
    }
  ...
  // Jika ada data riil, tampilkan grafik
  if (gridEl) gridEl.style.display = 'grid';
  if (emptyEl) emptyEl.style.display = 'none';

  // Chart 1: Cash flow (hanya dirender jika ada transaksi)
  ...
  // Chart 2: Utilitas / Beban (hanya dirender jika ada tagihan)
  ...
  if (ctx2 && hasBillData) {
    ...
    rtExpensePieChartInstance = new Chart(ctx2, {
      ...
      data: {
        ...
        datasets: [{
          data: billValues,
          backgroundColor: ['#f59e0b', '#0ea5e9', '#8b5cf6', '#10b981', '#ec4899', '#6366f1'],
          borderWidth: 2
        }]
      },
      options: {
...
function renderRumahTanggaKasKecilTable() {
  ...
  if (list.length === 0) {
    tbody.innerHTML = '<tr><td colspan="9" style="text-align:center; padding:30px; color:var(--t3);"><i class="fas fa-wallet" style="margin-right:8px; opacity:0.5;"></i>Belum ada catatan transaksi kas kecil.</td></tr>';
    return;
  }
...
function renderRumahTanggaRABTable() {
  ...
  const list = (data.budgets || []).filter(b => Number(b.allocated || 0) > 0 || Number(b.realized || 0) > 0);

  if (list.length === 0) {
    tbody.innerHTML = '<tr><td colspan="8" style="text-align:center; padding:30px; color:var(--t3);"><i class="fas fa-file-invoice-dollar" style="margin-right:8px; opacity:0.5;"></i>Belum ada data anggaran & realisasi RAB.</td></tr>';
    return;
  }
...
function renderRumahTanggaUtilitasTable() {
  ...
  const list = (data.bills || []).filter(b => Number(b.amount || 0) > 0);

  if (list.length === 0) {
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding:30px; color:var(--t3);"><i class="fas fa-bolt" style="margin-right:8px; opacity:0.5;"></i>Belum ada data tagihan utilitas kampus.</td></tr>';
    return;
  }
...
function renderRumahTanggaATKTable() {
  ...
  const list = (data.inventoryATK || []).filter(a => {
    const total = Number(a.stockAvailable || a.currentStock || a.stockInitial || 0);
    if (total === 0 && !q) return false;
    if (!q) return true;
  ...
  if (list.length === 0) {
    tbody.innerHTML = '<tr><td colspan="9" style="text-align:center; padding:30px; color:var(--t3);"><i class="fas fa-boxes-stacked" style="margin-right:8px; opacity:0.5;"></i>Belum ada data stok barang ATK.</td></tr>';
    return;
  }

  let rows = '';
  list.forEach(a => {
    const stock = Number(a.stockAvailable || a.currentStock || a.stockInitial || 0);
    const min = Number(a.minStock || 0);
    const price = Number(a.unitPrice || 0);
    const totalVal = stock * price;
    const isAman = stock >= min;

    rows += `<tr>
      <td><span class="badge" style="background:#8b5cf618; color:#8b5cf6; font-family:monospace; font-weight:700;">${a.code || '-'}</span></td>
      <td>
        <div style="font-weight:700; color:var(--t1);">${a.name || '-'}</div>
        <div style="font-size:0.75rem; color:var(--t3);">Lokasi: ${a.location || '-'}</div>
      </td>
      <td><span class="badge bg-p2" style="font-size:0.75rem;">${a.category || '-'}</span></td>
      <td style="font-weight:600;">${a.unit || 'Unit'}</td>
      <td style="text-align:center; color:var(--t3);">${min}</td>
      <td style="text-align:center; font-weight:800; color:${isAman ? '#10b981' : '#ef4444'}; font-size:1rem;">${stock}</td>
      <td style="text-align:right;">Rp ${price.toLocaleString('id-ID')}</td>
      <td style="text-align:right; font-weight:700; color:var(--t1);">Rp ${totalVal.toLocaleString('id-ID')}</td>
      <td>
        <span class="badge" style="background:${isAman ? '#10b98118' : '#ef444418'}; color:${isAman ? '#10b981' : '#ef4444'}; font-weight:700;">
          <i class="fas ${isAman ? 'fa-check' : 'fa-triangle-exclamation'}"></i> ${isAman ? 'Aman' : 'Menipis'}
        </span>
      </td>
    </tr>`;
  });
...
function exportRumahTanggaExcel() {
  ...
  // Sheet 1: Kas Kecil
  const kasData = (data.pettyCashTransactions || []).map(t => ({
  ...
  const ws1 = XLSX.utils.json_to_sheet(kasData);
  ...
  const atkData = (data.inventoryATK || []).map(a => ({
    ...
    'Kategori': a.category,
    'Satuan': a.unit,
    'Stok Tersedia': a.stockAvailable || a.currentStock || a.stockInitial,
    'Stok Min': a.minStock,
    'Harga Satuan': a.unitPrice,
    'Total Nilai': (a.stockAvailable || a.currentStock || a.stockInitial || 0) * (a.unitPrice || 0),
    'Lokasi': a.location
  }));
...
async function syncRumahTanggaFromSumber(silent = false) {
  ...
  if (!silent && typeof toast === 'function') {
    toast('Menyambungkan realtime ke Cloud Firestore Bidang Rumah Tangga...', 'info');
  }
  ...
  try {
    ...
    if (dbRumahTanggaSumber) {
      const docRef = dbRumahTanggaSumber.collection('sim_rt_database').doc('current_state');

      // 1. Pasang Realtime onSnapshot Listener agar selalu sinkron secara otomatis
      if (!window._rtCloudListenerAttached) {
        window._rtCloudListenerAttached = true;
        docRef.onSnapshot((snapshot) => {
          if (snapshot.exists) {
            const docData = snapshot.data();
            const payload = docData ? (docData.payload || docData) : null;
            if (payload) {
              saveSimRTData(payload);
            }
          } else {
            saveSimRTData({ pettyCashTransactions: [], budgets: [], bills: [], inventoryATK: [], cashOpnameReports: [] });
          }
          renderRumahTanggaContent();
        }, (err) => {
          console.warn('[Sync RT] Realtime listener notice:', err.message);
        });
      }
    ...
    renderRumahTanggaContent();
    if (!silent && typeof toast === 'function') {
      toast('Sinkronisasi Realtime Bidang Rumah Tangga Aktif!', 'success');
    }
  } catch(err) {
    console.warn('Sync RT notice:', err);
    renderRumahTanggaContent();
  } finally {
    if (icon) icon.className = 'fas fa-rotate';
    if (sbIcon) sbIcon.className = 'fas fa-rotate';
...
// Auto-sync realtime data rumah tangga saat SIMARSIP dimuat
setTimeout(() => {
